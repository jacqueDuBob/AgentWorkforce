...
export async function loadColumnAgents(): Promise<ColumnAgent[]> {
  ...
  return defaults().map((fallback) => {
    ...
    return row ? { id: row.id, column: fallback.column, name: row.name, instructions: row.instructions, startMode: row.start_mode, enabled: row.enabled, repositoryAccess: row.repository_access ?? "all", allowedRepositoryIds: (permissions ?? []).filter((item) => item.column_agent_id === row.id).map((item) => item.repository_id) } : fallback;
  });
}
...
export async function saveColumnAgent(agent: ColumnAgent) {
  if (!supabase) return;
  await ensureSupabaseSession();
  const { data, error } = await supabase.from("column_agents").upsert({ column_name: agent.column, name: agent.name, instructions: agent.instructions, start_mode: agent.startMode, enabled: agent.enabled, repository_access: agent.repositoryAccess }, { onConflict: "user_id,column_name" }).select("id").single();
  if (error) throw error;
