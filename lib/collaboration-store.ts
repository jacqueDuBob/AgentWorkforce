...
import type { AgentQuestion, Notification, TicketComment, TicketProposal } from "./collaboration-types";
...
export async function loadTicketCollaboration(ticketId: string) {
  ...
  const [{ data: comments, error: commentsError }, { data: questions, error: questionsError }, { data: proposals, error: proposalsError }] = await Promise.all([
  ...
  if (commentsError || questionsError || proposalsError) throw commentsError ?? questionsError ?? proposalsError;
  return { comments: (comments ?? []).map(comment), questions: (questions ?? []).map(question), proposals: (proposals ?? []).map(proposal) };
}
