...
export function QueueDialog({ tickets, onClose }: { tickets: Ticket[]; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="queue-dialog" role="dialog" aria-modal="true" aria-labelledby="queue-title" onMouseDown={(event) => event.stopPropagation()}>
    <header className="queue-header">
      ...
      <div>{!selected && <><label className="queue-filter"><span className="sr-only">Filter runs by status</span><select value={status} onChange={(event) => setStatus(event.target.value as AgentRunStatus)}><option value="queued">Queued</option><option value="in_progress">In progress</option><option value="waiting_for_input">Waiting for input</option><option value="resumable">Resuming</option><option value="blocked">Blocked</option><option value="finished">Finished</option></select></label><button className="icon-button" onClick={() => void refresh()} aria-label="Refresh queue"><RefreshCw size={17}/></button></>}<button className="icon-button" onClick={onClose} aria-label="Close"><X size={18}/></button></div>
    </header>
