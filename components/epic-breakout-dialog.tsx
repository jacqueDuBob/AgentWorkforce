...
export function EpicBreakoutDialog({ ticket, recommendation, agentName, domain, running, outcome, error, onClose, onConfirm }: {
  ticket: Ticket; recommendation: EpicRecommendation; agentName: string; domain: string; running: boolean; outcome?: BreakoutOutcome; error: string; onClose: () => void; onConfirm: () => void;
}) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={running ? undefined : onClose}><section className="epic-dialog" role="dialog" aria-modal="true" aria-labelledby="epic-title" onMouseDown={(event) => event.stopPropagation()}>
    <header className="refinement-header"><span><GitBranch size={20}/></span><div><p className="eyebrow">Epic candidate</p><h2 id="epic-title">{ticket.title}</h2></div><button className="icon-button" disabled={running} onClick={onClose} aria-label="Close"><X size={19}/></button></header>
    <div className="epic-body">
      {!outcome && <><div className="epic-recommendation"><Bot size={18}/><div><strong>{recommendation.recommendedBy} recommends converting this item to an Epic</strong><p>{recommendation.reason}</p></div></div><dl className="breakout-facts"><div><dt>Breakout agent</dt><dd>{agentName}</dd></div><div><dt>Repository / app domain</dt><dd>{domain}</dd></div><div><dt>Participant</dt><dd>You (requesting user)</dd></div></dl><p className="epic-warning">Confirmation updates this existing item in place, preserving its ID and history, and immediately starts one breakout session.</p></>}
    ...
    <footer className="refinement-actions"><button className="button secondary" disabled={running} onClick={onClose}>{outcome ? "Close" : "Cancel"}</button>{!outcome && <button className="button primary" disabled={running} onClick={onConfirm}>{running ? "Starting breakout…" : "Confirm Epic & start breakout"}</button>}</footer>
  </section></div>;
}
