...
export function EpicBreakoutDialog({ ticket, recommendation, forced = false, agentName, domain, running, dismissing, outcome, error, onClose, onConfirm, onDismiss }: {
  ticket: Ticket; recommendation?: EpicRecommendation; forced?: boolean; agentName: string; domain: string; running: boolean; dismissing: boolean; outcome?: BreakoutOutcome; error: string; onClose: () => void; onConfirm: () => void; onDismiss: () => void;
}) {
  const busy = running || dismissing;
  return <div className="modal-backdrop" role="presentation" onMouseDown={busy ? undefined : onClose}><section className="epic-dialog" role="dialog" aria-modal="true" aria-labelledby="epic-title" onMouseDown={(event) => event.stopPropagation()}>
    <header className="refinement-header"><span><GitBranch size={20}/></span><div><p className="eyebrow">{forced ? "Epic structuring" : "Epic candidate"}</p><h2 id="epic-title">{ticket.title}</h2></div><button className="icon-button" disabled={busy} onClick={onClose} aria-label="Close"><X size={19}/></button></header>
    ...
    <footer className="refinement-actions"><div>{!forced && !outcome && <button className="button secondary" disabled={busy} onClick={onDismiss}>{dismissing ? "Rejecting…" : "Reject recommendation"}</button>}</div><div className="epic-primary-actions"><button className="button secondary" disabled={busy} onClick={onClose}>{outcome ? "Close" : "Cancel"}</button>{!outcome && <button className="button primary" disabled={busy} onClick={onConfirm}>{running ? "Structuring Epic…" : "Confirm Epic & start structuring"}</button>}</div></footer>
  </section></div>;
}
