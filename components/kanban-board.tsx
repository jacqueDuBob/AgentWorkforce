...
export function KanbanBoard({ userEmail, onSignOut }: { userEmail: string; onSignOut: () => void }) {
  ...
  useEffect(() => { const refreshNotifications = () => { void loadNotifications().then(setNotifications).catch(() => undefined); }; const interval = window.setInterval(refreshNotifications, 30000); return () => window.clearInterval(interval); }, []);
